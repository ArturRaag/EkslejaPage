...
var n=4; // v�rrandite arv diff v�rrandi s�steemis
...
function toggle_button() {
  if (toggle==true){
    ...
    STOP_CONTINUE_button.style('color','black');
        ...
        STOP_CONTINUE_button.html("J&aumltka");
...
canvas=createCanvas(800,500+graafikute_korgus);
canvas.parent('kepler_sketch');
background(0);
coefficients();
init_model(0);
...
STOP_CONTINUE_button.position(width-30, (height-graafikute_korgus)*0+30);
...
STOP_CONTINUE_button.parent('kepler_sketch');
  ...
  RESET.parent('kepler_sketch');

EARTH_button = createButton("Maa");
EARTH_button.size(100,50);
EARTH_button.position(width-30, (height-graafikute_korgus)*0+100);
...
EARTH_button.parent('kepler_sketch');

MARS_button = createButton("Mars");
MARS_button.size(100,50);
MARS_button.position(width-30, (height-graafikute_korgus)*0+160);
...
MARS_button.parent('kepler_sketch');


VENUS_button = createButton("Veenus");
VENUS_button.size(100,50);
VENUS_button.position(width-30, (height-graafikute_korgus)*0+220);
...
VENUS_button.parent('kepler_sketch');
...
Vy_input.value(y[3]);
Vy_input.size(80,15);
X_coord_input.parent('kepler_sketch');
Y_coord_input.parent('kepler_sketch');
Vx_input.parent('kepler_sketch');
Vy_input.parent('kepler_sketch');
...
Xinput_text.parent('kepler_sketch');
...
Yinput_text.parent('kepler_sketch');
...
Vxinput_text.parent('kepler_sketch');
...
Vyinput_text.parent('kepler_sketch');  
...
WARNING_TEXT.parent('kepler_sketch');  
  
algtingimused_text=createP("Algtingimused");
algtingimused_text.position(width+50,(height-graafikute_korgus)*0+290);
algtingimused_text.style("font: Times New Roman");
algtingimused_text.style("font-size: 24px;");
algtingimused_text.parent('kepler_sketch');  
...
radio2.selected('3');
radio2.parent('kepler_sketch');  

MORE_OPTIONS_TEXT=createP("Muud s&aumltted");
...
MORE_OPTIONS_TEXT.parent('kepler_sketch');  
...
PLOT_SCALE_SLIDER.parent('kepler_sketch'); 
PLOT_SCALE_SLIDER_TEXT.parent('kepler_sketch');
ANIM_SCALE_SLIDER.parent('kepler_sketch'); 
ANIM_TEXT.parent('kepler_sketch'); 
...
fill(255);
circle(width/2, (height-graafikute_korgus)/2 ,2*radius/(100*anim_scale)); // Planet
  push();
  fill(255,0,0);
circle((y[0]/(100*anim_scale))+width/2, -1*y[1]/(100*anim_scale)+(height-graafikute_korgus)/2 ,1000/(100*anim_scale)); // Sattelite
  pop();
...
text("Planeet: "+model_name, width/2, (height-graafikute_korgus)*0+50);
...
if ( Math.sqrt(y[0]*y[0]+y[1]*y[1])<=radius ) {
  ...
  STOP_CONTINUE_button.html("J�tka");
  STOP_CONTINUE_button.style("background-color","#4A88E0")
  STOP_CONTINUE_button.style('color','white');
...
function GRAAFIK_V_AEG() {
  for (var j=0; j<V_vaartused.length; j=j+1){
    push();
    textAlign(RIGHT);
    text(round_2(V_vaartused[j]*plot_scale) , plot_offset_X-15, height-25*j);
    pop();
  };
  push();
  textAlign(RIGHT);
  textSize(16);
  text("Vx(t)", plot_offset_X-35, height-graafikute_korgus/2);
  text("[km/s]", plot_offset_X-35, height-graafikute_korgus/2+15);
  ...
  for (var k=0; k<=aja_massiiv.length; k=k+1){
    if (abs(( Math.sqrt((Vx_massiiv[k]*Vx_massiiv[k])+(Vy_massiiv[k]*Vy_massiiv[k]))*10 )/plot_scale) <= graafikute_korgus/2) {
...
function GRAAFIK_A_AEG(){
  ...
  textAlign(RIGHT);
  textSize(16);
  text("Ax(t)", plot_offset_X-35, height-graafikute_korgus/2);
  text("[km/s2]", plot_offset_X-35, height-graafikute_korgus/2+15);
  text("t [s]",width-25, height-graafikute_korgus/2+20 );
  pop();
push();
stroke(255,125,0);
strokeWeight(1.5);
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if ( abs((Math.sqrt(Ax_massiiv[k]*Ax_massiiv[k]+Ay_massiiv[k]*Ay_massiiv[k])*1000)/plot_scale) <= graafikute_korgus/2 ) {
...
function GRAAFIK_E_tot_AEG(){
    ...
    push();
    textAlign(RIGHT);
    text(round_4((V_vaartused[j])*10*plot_scale) , plot_offset_X-15, height-25*j);
    pop();
  };
  push();
  textAlign(RIGHT);
  textSize(16);
  text("E(t)", plot_offset_X-35, height-graafikute_korgus/2);
  ...
  textAlign(LEFT);
  textSize(14);
  text("1e8", plot_offset_X+10, height-graafikute_korgus+15);
  pop();
...
strokeWeight(1.5);
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Etot_massiiv[k]/1e8)/plot_scale) <= graafikute_korgus/2 ) {
    push();
    fill(255);
    stroke(255,125,0);
    strokeWeight(1);
