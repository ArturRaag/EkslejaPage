...
//_____________________________________________________________________________________
function setup() {
  canvas=createCanvas(rl[0]*scaler+500, rl[1]*scaler+graafikute_kõrgus);
  canvas.parent('LJP_sim_sketch');
  ...
  STOP_CONTINUE_button.parent('LJP_sim_sketch');
  ...
  SLOW_DOWN.parent('LJP_sim_sketch');
  ...
  Temperature_text.style("color: bgColor");
  Temperature_text.style("font-size: 16px");
  TEMP_SLIDER.parent('LJP_sim_sketch');
  Temperature_text.parent('LJP_sim_sketch');
  ...
  Gravity_text.style("color: bgColor");
  Gravity_text.style("font-size: 16px");
  GRAVITY_SLIDER.parent('LJP_sim_sketch');
  Gravity_text.parent('LJP_sim_sketch');
  
  text_for_init=createP("For initializations");
  text_for_init.style("font-family: Times New Roman, Times, serif");
  text_for_init.style("color: bgColor");
  ...
  text_for_init.parent('LJP_sim_sketch');
  radio_init.parent('LJP_sim_sketch');
  ...
  gravity_checkbox.parent('LJP_sim_sketch');
  ...
  temp_checkbox.parent('LJP_sim_sketch');
  ...
  single_checkbox.parent('LJP_sim_sketch');
  ...
  trail_checkbox.parent('LJP_sim_sketch');
  ...
  etot_checkbox.parent('LJP_sim_sketch');
  ...
  velocity_checkbox.parent('LJP_sim_sketch');
  ...
  temporary_energy.parent('LJP_sim_sketch');
...
pop();
  ...
  //text("[J]", plot_oGffset_X-35, height-graafikute_kõrgus/2+15);
            ...
            for (k=0;k<particles-1;k++)
            {
               for (i=k+1;i<particles;i++)
               {
                  ...
                  if(dist < Math.pow(rcut,2)){
                     rd=1./Math.pow(dist,6)-1./Math.pow(dist,3);
                     ep=ep+rd;
                  }
                  
               }
               ek=ek+Math.pow(vx[k],2)+Math.pow(vy[k],2);
               ep=ep-6.*y[k]*ag;
            }
            
        //    ek=ek+Math.pow(vx[particles-1],2)+Math.pow(vy[particles-1],2);
	//    ep=ep+6.*y[particles-1]*ag;
            ek=ek*3.
            etot=-(ek+ep);
}
