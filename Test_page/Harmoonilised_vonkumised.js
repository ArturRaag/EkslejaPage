...
var graafikute_korgus=300;
var plot_offset_X=100;
var plot_offset_Y=graafikute_korgus/2;
...
var n=2; // v�rrandite arv diff v�rrandi s�steemis
...
// k_{i,n} masiiv, i=k_{i} ja n= v�rrandi number
for (var i=0; i<6; i++){
k[i]= new Array(n);}
...
a[5]=1.0/2.0;
 ...
 // Alguses oli massiv b initsialiseeritud kui 1 veeruline maatriks, n��d on tal 6 rida ja 5 veergu
// T�idame B massiivi �ra Fehlbergi koefitsentidega
b[0][0]=0;
b[0][1]=1.0/4.0;
b[0][2]=3.0/32.0;
b[0][3]=1932.0/2197.0;
...
yp[1]=-omega_ruut*yy[0];
  ...
  yp[0]=yy[1];
  yp[1]=-2*beta*yy[1]-omega_ruut*yy[0];
}
}
...
//  if (te[neq]>e ) {
//  h=h_uus;
//  neq=neq-1; // Kordame iteratsiooni j�rgmisel iteratsioonil
//  } else if (te[neq]<= e){
//  h=h_uus; // iteratsiooni ei korda, l�hme edasi.
//  }
//  }
...
function setup() {
  ...
  canvas=createCanvas(900,400+graafikute_korgus);
  canvas.parent('HV_sim_sketch');
  coefficients();
  
  STOP_CONTINUE_button = createButton("Start");
  STOP_CONTINUE_button.size(100,50);
  STOP_CONTINUE_button.position(width-40, 30);
  ...
  STOP_CONTINUE_button.parent('HV_sim_sketch');
  
  KESKKONNA_OMDAUSED_TEXT=createP("Keskkonna<br>omadused");
  KESKKONNA_OMDAUSED_TEXT.position(width+50, 110);
  KESKKONNA_OMDAUSED_TEXT.style("font-weight","bold");
  KESKKONNA_OMDAUSED_TEXT.style("font-size","24px");
  KESKKONNA_OMDAUSED_TEXT.parent('HV_sim_sketch');

  MODEL_1_BUTTON=createButton("Harmooniline");
  MODEL_1_BUTTON.style('padding','10px 20px');
  MODEL_1_BUTTON.style('background-color','MidNightBlue');
  MODEL_1_BUTTON.style('color','white');
  MODEL_1_BUTTON.style('font-weight','bold');
  MODEL_1_BUTTON.style('border-radius','30px');
  MODEL_1_BUTTON.style('margin-top','30px');
  MODEL_1_BUTTON.style('margin-left','80px');
  MODEL_1_BUTTON.parent('HV_sim_sketch');

  
  MODEL_2_BUTTON=createButton("Sumbvõnkumised");
  MODEL_2_BUTTON.style('padding','10px 20px');
  MODEL_2_BUTTON.style('background-color','MidNightBlue');
  MODEL_2_BUTTON.style('color','white');
  MODEL_2_BUTTON.style('font-weight','bold');
  MODEL_2_BUTTON.style('border-radius','30px');
  MODEL_2_BUTTON.style('margin-top','30px');
  MODEL_2_BUTTON.style('margin-left','80px');
  MODEL_2_BUTTON.parent('HV_sim_sketch');

  MODEL_1_BUTTON.position(width-40,170);
  MODEL_2_BUTTON.position(width-40,220);
  
  
  MODEL_1_BUTTON.size(160,40);
  MODEL_2_BUTTON.size(160,40);
  ...
  MASS_SLIDER.position(width+50,330);
  MASS_SLIDER_TEXT=createP("Mass: "+m+"kg");
  MASS_SLIDER_TEXT.position(width+50,300);
  MASS_SLIDER.parent('HV_sim_sketch');
  MASS_SLIDER_TEXT.parent('HV_sim_sketch');
  
  HOOKE_SLIDER=createSlider(0.01,9.0 , 5.0, 0.01);
  HOOKE_SLIDER.position(width+50,380);
  HOOKE_SLIDER_TEXT=createP("Vedru jäikus "+k_Hooke);
  HOOKE_SLIDER_TEXT.position(width+50,350);
  HOOKE_SLIDER.parent('HV_sim_sketch');
  HOOKE_SLIDER_TEXT.parent('HV_sim_sketch');
  
  ALPHA_SLIDER=createSlider(0,1,0.1,0.01);
  ALPHA_SLIDER.position(width+50,430);
  ALPHA_SLIDER_TEXT=createP("Takistusjõu tegur (alfa): "+ALPHA_SLIDER.value());
  ALPHA_SLIDER_TEXT.position(width+50,400);
  ALPHA_SLIDER.parent('HV_sim_sketch');
  ALPHA_SLIDER_TEXT.parent('HV_sim_sketch');
  
  GRAAFIKU_OMADUSED_TEXT=createP("Graafikute<br>omadused");
  GRAAFIKU_OMADUSED_TEXT.position(width+50, (height-graafikute_korgus)+70);
  GRAAFIKU_OMADUSED_TEXT.style("font-size","24px");
  GRAAFIKU_OMADUSED_TEXT.style("font-weight","bold");
  GRAAFIKU_OMADUSED_TEXT.parent('HV_sim_sketch');
  
  radio2 = createRadio();
  radio2.position(width+50, (height-graafikute_korgus)+160);
  ...
  radio2.selected('1');
  radio2.parent('HV_sim_sketch');
  
  PLOT_SCALE_SLIDER=createSlider(0.01, 5, 1, 0.1);
  PLOT_SCALE_SLIDER.position(width+50,height-50);
  PLOT_SCALE_SLIDER_TEXT=createP("Graafiku skaala");
  PLOT_SCALE_SLIDER_TEXT.position(width+50, height-80);
  PLOT_SCALE_SLIDER.parent('HV_sim_sketch');
  PLOT_SCALE_SLIDER_TEXT.parent('HV_sim_sketch');
...
function draw() {
  ...
  if (mudel_num==1){
    MODEL_1_BUTTON.style('background-color','MidNightBlue');
    MODEL_2_BUTTON.style('background-color','white');
    MODEL_1_BUTTON.style('color','white');
    MODEL_2_BUTTON.style('color','black');
  } else if (mudel_num==2){
    MODEL_1_BUTTON.style('background-color','white');
    MODEL_2_BUTTON.style('background-color','MidNightBlue');
    MODEL_1_BUTTON.style('color','black');
    MODEL_2_BUTTON.style('color','white');
  }
  ...
  if (locked==true){
    circle(mouseX,(height-graafikute_korgus)/2,20);
    push();
    fill(255);
    textSize(18);
  ...
  //console.log(y[0],y[1]);
  circle(y[0]*10+width/2,(height-graafikute_korgus)/2,20);
    push();
    fill(255);
    text("nihe: "+nihe+"m",width/2,30);
    pop();
...
//fill(32,42,68);
fill(25);
rect(0,height-graafikute_korgus,width,height);
pop();
...
line(width-10,height-(plot_offset_Y-5),width,height-plot_offset_Y);
line(plot_offset_X,height,plot_offset_X,height-graafikute_korgus); // Y-axis
line(plot_offset_X-5,height-graafikute_korgus+10,plot_offset_X, height-graafikute_korgus);
line(plot_offset_X+5,height-graafikute_korgus+10,plot_offset_X, height-graafikute_korgus);
for (j=0; j < 12; j=j+1 ) {
  push();
  stroke(255);
...
function mouseDragged(){
  if (mouseX<=width && mouseX>=0 && mouseY<=height-graafikute_korgus && mouseY >=0){
  locked=true;
...
var V_vaartused=[-15 ,-12.5 ,-10 ,-7.5 ,-5 ,-2.5 ,0 ,2.5 ,5 ,7.5 ,10 ,12.5];
function GRAAFIK_E_tot_AEG(){
    for (var j=0; j<V_vaartused.length; j=j+1){
    push();
    fill(255);
    textAlign(RIGHT);
    text(round_4((V_vaartused[j])*100*plot_scale) , plot_offset_X-15, height-25*j);
    pop();
  };
  push();
  fill(255);
  textAlign(RIGHT);
  textSize(16);
  text("ET(t)", plot_offset_X-35, height-graafikute_korgus/2);
  text("[J]", plot_offset_X-35, height-graafikute_korgus/2+15);
  text("t [s]",width-25, height-graafikute_korgus/2+20 );
  pop();
  push();
  textAlign(LEFT);
  textSize(14);
  text("", plot_offset_X+10, height-graafikute_korgus+15);
  pop();
push();
stroke(255,125,0);
strokeWeight(1.5);
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Etot_massiiv[k]/10)/plot_scale) <= graafikute_korgus/2 ) {
    if (k>=1) {
...
function GRAAFIK_E_kin_AEG(){
  ...
  textAlign(RIGHT);
  textSize(16);
  text("EK(t)", plot_offset_X-35, height-graafikute_korgus/2);
  text("[J]", plot_offset_X-35, height-graafikute_korgus/2+15);
  text("t [s]",width-25, height-graafikute_korgus/2+20 );
  pop();
  push();
...
strokeWeight(1.5);
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Ekin_massiiv[k]/10)/plot_scale) <= graafikute_korgus/2 ) {
    if (k>=1) {
...
function GRAAFIK_E_pot_AEG(){
  ...
  textAlign(RIGHT);
  textSize(16);
  text("EP(t)", plot_offset_X-35, height-graafikute_korgus/2);
  text("[J]", plot_offset_X-35, height-graafikute_korgus/2+15);
  text("t [s]",width-25, height-graafikute_korgus/2+20 );
  pop();
  push();
...
strokeWeight(1.5);
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Epot_massiiv[k]/10)/plot_scale) <= graafikute_korgus/2 ) {
    if (k>=1) {
